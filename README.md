...
### 1. Weekly Log — 내가 한 일의 기록
```
docs/weekly-logs/weekly_log_2026-04-27.md
```
- **언제**: 업무 얘기를 할 때마다
- **무엇**: 처리한 이슈, 회의 내용, 할 일 목록
- **누가 씀**: Claude가 자동으로 작성
- **내가 볼 수 있나**: 네, 그냥 파일을 열면 됩니다
...
```
- **언제**: Claude가 "이건 다음에도 기억해야겠다" 싶을 때
- **무엇**: 내 역할, 선호하는 답변 방식, 진행 중인 프로젝트 맥락
- **누가 씀**: Claude가 알아서 판단해서 저장
- **내가 볼 수 있나**: 네, 파일로 저장되기 때문에 확인·수정 가능

### 차이점 한 줄 요약

| | Weekly Log | Memory |
|---|---|---|
| 성격 | 내가 한 일 (업무 일지) | 나에 대한 정보 (맥락) |
| 트리거 | 업무 얘기를 할 때 | Claude가 판단 |
| 단위 | 주차별 파일 | 주제별 파일 |

> 두 가지 모두 **Claude Code가 켜져 있는 동안**에만 쌓입니다.
> 백그라운드에서 자동으로 돌아가는 프로그램이 아닙니다.
